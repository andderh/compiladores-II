...
public abstract class Semantico {
    ...
    public static boolean run() {
        MainLogger.logInfo("[" + Helper.getTimeStamp() + "] Iniciando análise semântica");
    ...
    public static void CheckSemanticAtrib() {
        log.log(Priority.DEBUG, "Iniciando análise semântica de atribuição");
        while ((pilha_expressao.size() >= 1) && (pilha_identificadores.size() >= 1)) {
            Token t1 = (Token) pilha_identificadores.pop();
            Token t2 = (Token) pilha_expressao.pop();
            if (t1.getType() == t2.getType()) {
                pilha_expressao.push(t2);
            } else {
                error_s(TokensList.getActualLine(), "Atribuição com incompatibilidade de tipo. Pois [" + t1.getToken() + "] é do tipo [" + t1.getType() + "] e [" + t2.getToken() + "] é do tipo [" + t2.getType() + "]!");
            }
        }
    }
    ...
    public static void CheckSemanticExp() {
        ...
        while (pilha_expressao.size() >= 2) {
            ...
            if (t1.getType() == t2.getType()) {
                ...
                error_s(TokensList.getActualLine(), "Expressão com incompatibilidade de tipo. Pois [" + t1.getToken() + "] é do tipo [" + t1.getType() + "] e [" + t2.getToken() + "] é do tipo [" + t2.getType() + "]!");
            }
        }
        CheckSemanticAtrib();
    }
    ...
    public static void CheckSemanticParReaisUsr(Contexto contexto) {
        log.log(Priority.DEBUG, "Iniciando análise semântica dos parâmetros reais e formais");
        if (pilha_par_reais_proc_usuario.size() >= 0 && pilha_proc_usuario.size() >= 1) {
            ...
            if (par_r.length == par_f.size()) {
                ...
                for (int i = 0; i < par_r.length; i++) {
                    ...
                    if (!par_r_1_t.tipo.equals(par_f_1.tipo)) {
                        error_s(par_r_1.getLine(), "Incompatibilidade de tipo entre parâmetros formais e reais. "
                                + "O procedimento [" + proc.ident + "] em seu " + (i + 1) + "º parâmetro esperava o tipo [" + par_f_1.tipo + "] e encontrou "
                                + "o parâmetro [" + par_r_1_t.ident + "] do tipo [" + par_r_1_t.tipo + "] ");
                    }
                }
            } else {
                error_s(TokensList.getActualLine() - 1, "Incompatibilidade de número de parâmetros."
                        + " O procedimento [" + proc.ident + "] possui " + par_f.size() + " parâmetro(s) formal(ais) "
                        + " e encontrou " + par_r.length + " parâmetro(s) real(ais)]");
            }
        }
    ...
    public static void preenchePilha(Stack pilha, Contexto contexto) {
        ...
        if (t.getType() == TypeClass.Identificadores) {
            /*Percorrendo o contexto do escopo mais interno para o mais externo ver se o token é alguma variável declarada e qual é seu tipo*/
            Token temp_t = t;
            ...
            if (var.tipo == null) {
                ...
                System.out.println("RESGATE: PAR:" + var.ident + "TIPO:" + var.tipo);
                if (var.tipo != null) {
                    if (var.tipo.equals("real")) {
                        ...
                        log.log(Priority.ERROR, "Tipo da parâmetro [" + t.getToken() + "] não existente!");
                    }
                } else {
                    error_s(TokensList.getActualLine(), "Variável [" + t.getToken() + "] não declarada!");
                ...
                if ((contexto.request_par(t.getToken(), contexto.escopo).tipo != null) && (contexto.request_var_local(t.getToken(), contexto.escopo).tipo != null)) {
                    System.out.println("FOI NA PILHA DE " + pilha.toString());
                    error_s(TokensList.getActualLine(), "Variavél [" + t.getToken() + "] está em conflito com parâmetro [" + t.getToken() + "]!");
                }
                System.out.println("RESGATE: VAR:" + var.ident + "TIPO:" + var.tipo);
                ...
                if (var.tipo != null) {
                    if (var.tipo.equals("real")) {
                        ...
                        log.log(Priority.ERROR, "Tipo da variavel [" + t.getToken() + "] não existente!");
                    }
                } else {
                    error_s(TokensList.getActualLine(), "Variavél [" + t.getToken() + "] não declarada!");
                    t.setType(TypeClass.ERRO);
                }
                if (t.getType() == TypeClass.ERRO) {
                    Semantico.pilha_erro.push(temp_t);
    ...
    public static void preenchePilhaIdent(Contexto contexto) {
        ...
        log.log(Priority.DEBUG, "::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::ANTANT:" + ant.getToken() + " ANT:" + actual.getToken());
                ...
                if (var.tipo == null) {
                    ...
                    if (var.tipo != null) {
                        if (var.tipo.equals("real")) {
                            ...
                            log.log(Priority.ERROR, "Tipo da parâmetro [" + ant.getToken() + "] não existente!");
                        }
                    } else {
                        error_s(TokensList.getActualLine(), "Variável [" + ant.getToken() + "] não declarada!");
                    ...
                    if ((contexto.request_par(ant.getToken(), contexto.escopo).tipo != null) && (contexto.request_var_local(ant.getToken(), contexto.escopo).tipo != null)) {
                        System.out.println("FOI NA PILHA DE IDENTIFICADOR");
                        error_s(TokensList.getActualLine(), "Variavél [" + ant.getToken() + "] está em conflito com parâmetro [" + ant.getToken() + "]!");
                    }
                    System.out.println("RESGATE: VAR:" + var.ident + "TIPO:" + var.tipo);
                    ...
                    if (var.tipo != null) {
                        if (var.tipo.equals("real")) {
                            ...
                            log.log(Priority.ERROR, "Tipo da variavel [" + ant.getToken() + "] não existente!");
                        }
                    } else {
                        error_s(TokensList.getActualLine(), "Variavél [" + ant.getToken() + "] não declarada!");
                        ant.setType(TypeClass.ERRO);
                    }
                    if (ant.getType() == TypeClass.ERRO) {
                        Semantico.pilha_erro.push(temp_t);
    ...
    public static void preenchePilhaProc(Contexto contexto) {
        ...
        if (t.getType() == TypeClass.Identificadores) {
            ...
            if (proc.ident != null) {
                ...
                error_s(TokensList.getActualLine(), "Procedimento [" + t.getToken() + "] não declarado!");
            }
